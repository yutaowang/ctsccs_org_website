...
export function tuitionAfterWaterfordDiscount(course, studentId, seats) {
  const tuition = Number(course?.donation || 0);
  if (!hasFreeWaterfordSeat(course, studentId, seats)) return tuition;
...
export function tuitionForCourses(courses, studentId, seats) {
  return [...new Map(courses.map((course) => [course.id, course])).values()]
    .reduce((sum, course) => sum + tuitionAfterWaterfordDiscount(course, studentId, seats), 0);
}
